...
| `lauren-logging` | Configurable logging module with processor pipeline, contextvars binding, request-logging middleware, and pluggable backends (stdlib, structlog, console, file, fan-out, queue, …). |

When fixing a logging or middleware concern, **first check whether the
right home is the companion package**, not core. The framework stays
